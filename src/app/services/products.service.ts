...
export class ProductsService {
  ...
  filterTypeEmitter = new EventEmitter<string>(); // emitted when filtering through product categories
  ...
  fetchProductsFromDB(): Observable<any> {
    return this.httpClient.get(`${environment.apiBase}`)
      .map(productSingle => {
  ...
  fetchSingleProductFromDB(indexID: string): Observable<any> {
    return this.httpClient.get(`${environment.apiBase}${indexID}`)
      .map(singleProduct => {
